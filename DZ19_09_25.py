...
pipe = download_model()
def generate_image(prompt, pipe, steps=20, guidance=7.5, width=512, height=512, seed=None, negative_prompt=None, save_path="outputs"):
    if seed is not None:
    ...
    filename = f"{save_path}/{prompt[:20]}_steps{steps}_guid{guidance}_size{width}x{height}.png"
    image.save(filename)
