...
# abs_diff = conv_rates["B"] - conv_rates["A"]
# rel_change = abs_diff / conv_rates["A"]
#
# print("Конверсії по групах:")
# print(conv_rates)
# print(f"Абсолютна різниця: {abs_diff:.3f}")
# print(f"Відносна зміна: {rel_change:.2%}")
#
# def ci_binomial(n_converted, n_total, alpha=0.05):
#     p = n_converted / n_total
#     se = np.sqrt(p*(1-p)/n_total)
#     z = stats.norm.ppf(1 - alpha/2)
#     return p - z*se, p + z*se
#
# ci_A = ci_binomial(converted_A.sum(), n_A)
# ci_B = ci_binomial(converted_B.sum(), n_B)
#
# print(f"95% CI для групи A: {ci_A}")
# print(f"95% CI для групи B: {ci_B}")
#
# groups = ["A", "B"]
# rates = [conv_rates["A"], conv_rates["B"]]
# cis = [[conv_rates["A"] - ci_A[0], ci_A[1] - conv_rates["A"]],
#        [conv_rates["B"] - ci_B[0], ci_B[1] - conv_rates["B"]]]
#
# plt.bar(groups, rates, yerr=np.array(cis).T, capsize=5, color=['skyblue', 'salmon'])
# plt.ylabel("Конверсія")
# plt.title("Конверсії A/B-груп з 95% довірчими інтервалами")
# plt.ylim(0, 1)
# plt.show()
